...
const BLOG = ({ header, link, blogList, headerDivider = false }) => {
  ...
  useEffect(() => {
    const handleResize = () => {
      if (window.innerWidth >= 1024) {
        setVisibleBlogs(3); // Large screens
      } else if (window.innerWidth >= 768) {
        setVisibleBlogs(4); // Tablets
      } else {
        setVisibleBlogs(3); // Small screens
      }
    };
  ...
  const blog = blogList
    ? blogList
    : allBlogs.slice(0, visibleBlogs).map((item) => ({
        ...item,
        truncatedBody:
  ...
  return (
    <div className="md:px-[3rem] py-6 px-6 lg:px-[2rem] box-border home-blog-container">
      ...
      {!blog.length ? (
        <p className="text-center text-[1.4rem] text-darkblue animate-pulse py-20">
          No blogs available!
        </p>
      ) : (
      ...
      {link && (
        <div
          className="flex sm:justify-end justify-center items-center mt-6 sm:items-end mb-6"
          onClick={scrollToTop}
        >
          <Link to={link}>
